...
{
    term_t *temp = malloc(sizeof(term_t));
	assert(temp != NULL && coeff != 0);
	//assert(exp > 0);
	temp -> coeff = coeff;
	temp -> exp = exp;
	return temp;
...
{
    polynomial_t *temp = malloc(sizeof(polynomial_t));
	assert(temp != NULL);
	temp->num_terms = 0;
	return temp;
}
...
double eval_polynomial(polynomial_t *poly, double x)
{
	...
	for(int c = 0; c < poly->num_terms; c++)
	{
		val+=eval_term(poly->terms[c], x);
	}   
	return val;
}
